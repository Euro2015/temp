...
class AccountBalance extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('user_id, account_balance', 'required'),
			array('user_id', 'numerical', 'integerOnly'=>true),
			array('account_balance', 'length', 'max'=>10),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, user_id, account_balance', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('account_balance',$this->account_balance,true);
