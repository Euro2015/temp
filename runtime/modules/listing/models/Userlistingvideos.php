...
class Userlistingvideos extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('drg_videodesc, drg_listing_video, drg_lid', 'required'),
			array('drg_lid', 'numerical', 'integerOnly'=>true),
			array('drg_videodesc', 'length', 'max'=>250),
			array('drg_listing_video', 'length', 'max'=>500),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('drg_video_id, drg_videodesc, drg_listing_video, drg_lid', 'safe', 'on'=>'search'),
		);
	}
